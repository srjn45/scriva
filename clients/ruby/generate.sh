#!/usr/bin/env bash
# generate.sh — regenerate Ruby gRPC stubs from proto/scriva.proto.
#
# Requirements:
#   gem install grpc-tools   (provides grpc_tools_ruby_protoc)
#
# Output goes to lib/scriva/proto/ — commit the results.
#
...
# grpc_tools emits a bare `require 'scriva_pb'` in the service stub, which only
# resolves if OUT_DIR happens to be on the load path. Rewrite it to the
# package-relative path so `require "scriva/proto/scriva_services_pb"` works
# from anywhere on the gem's load path.
SERVICES_FILE="$OUT_DIR/scriva_services_pb.rb"
if [ -f "$SERVICES_FILE" ]; then
  sed -i.bak "s#^require 'scriva_pb'#require 'scriva/proto/scriva_pb'#" "$SERVICES_FILE"
