...
| 8 | Kotlin | Maven Central (`io.github.srjn45:scriva-client-kotlin`) | `clients/kotlin/` | ✅ Done |
| 9 | Scala | Maven Central (`io.github.srjn45:scriva-client-scala`) | `clients/scala/` | ✅ Done |
| 10 | Clojure | Clojars (`io.github.srjn45/scriva-client-clojure`) | `clients/clojure/` | ✅ Done |
...
## 8–10 — JVM SDKs (Kotlin, Scala, Clojure)

Three additional JVM clients, each generating its own gRPC stubs from
`proto/scriva.proto` (mirroring the Java client) and exposing the same functional
surface — collection management, CRUD, keyed CRUD / upsert / CAS (N1), field
projection (N2), multi-field ordering + keyset pagination (N3), streaming
aggregations (N4), indexes, transactions, watch, stats, compact and snapshot —
idiomatically per language.

### 8 — Kotlin (`clients/kotlin/`, Gradle)
- [x] `grpc-kotlin` + coroutines; unary calls are `suspend`, streaming `find`/`watch`/`aggregate`/`snapshot` return `Flow`.
- [x] Sealed `Filter` hierarchy (`field` / `and` / `or`); `Record`/`Page`/`AggResult`/… as data classes; typed `NotFoundException` / `AlreadyExistsException`.
- [x] Proto stubs generated via the `com.google.protobuf` Gradle plugin from `../../proto`.
- [x] Hermetic in-process gRPC test + a runnable `BasicExample`.
- [x] Publishes `io.github.srjn45:scriva-client-kotlin:1.2.1` to Maven Central (vanniktech + Central Portal, mirrors Java).

### 9 — Scala (`clients/scala/`, sbt + ScalaPB)
- [x] Unary RPCs return `scala.concurrent.Future`; `find` streams via `Iterator` (`findStream`) or collects to `Future[Seq]`; `watch` is a cancellable callback.
- [x] Sealed `Filter` / `FilterOp`; case-class `Record`/`Page`/…; typed exceptions.
- [x] Proto stubs generated by ScalaPB from `../../proto`; Scala 2.13.
- [x] Hermetic in-process ScalaTest suite + a runnable `BasicExample`.
- [x] Publishes `io.github.srjn45:scriva-client-scala_2.13:1.2.1` to Maven Central (sbt-sonatype Central Portal + sbt-pgp).

### 10 — Clojure (`clients/clojure/`, Leiningen)
- [x] Idiomatic functions over Clojure maps; streaming `find`/`watch`/`aggregate`/`snapshot` return lazy seqs.
- [x] Plain-data filters (`{:field .. :op .. :value ..}` / `{:and [..]}` / `{:or [..]}`); errors as `ex-info` with `:type :not-found` / `:already-exists`.
- [x] Java gRPC stubs generated at build time by `lein-protoc` from `../../proto` (with vendored `google/api` protos under `proto/`).
- [x] Hermetic in-process `clojure.test` suite + a runnable `scriva.example`.
- [x] Publishes `io.github.srjn45/scriva-client-clojure:1.2.1` to Clojars (needs `CLOJARS_USERNAME` / `CLOJARS_TOKEN` secrets).
