...
/**
 * Integration test for {@link ScrivaDBClient}.
 *
 * Requires a running ScrivaDB server on localhost:5433 with api-key "dev-key".
 * Start it with:
...
class ExampleTest {
    ...
    private static final String HOST    = System.getenv().getOrDefault("SCRIVA_HOST",    "localhost");
    private static final int    PORT    = Integer.parseInt(System.getenv().getOrDefault("SCRIVA_PORT",    "5433"));
    private static final String API_KEY = System.getenv().getOrDefault("SCRIVA_API_KEY", "dev-key");
    private static final String COL     = "test_java_integration";

    private static ScrivaDBClient db;
    ...
    @BeforeAll
    static void connect() {
        db = new ScrivaDBClient(HOST, PORT, API_KEY);
    ...
    void snapshotToFile() throws Exception {
        java.io.File backup = java.io.File.createTempFile("scriva_java_snapshot_test", ".tar.gz");
        try {
    ...
    void keysetPaginationAndMultiFieldOrder() {
        ...
        try {
            ...
            List<ScrivaDBClient.Order> order =
                    List.of(ScrivaDBClient.Order.asc("team"), ScrivaDBClient.Order.desc("score"));
