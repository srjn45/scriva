...
- **Read replicas** — a follower serves reads (`Find`/`FindById`/`FindByKey`/`Aggregate`) from its applied state and refuses writes with a typed `FAILED_PRECONDITION`, so read traffic scales horizontally across followers. Bound staleness by diffing a follower's `applied_lsn` against the leader's `leader_lsn`. Failover is the next milestone
